"""
CSMA + 冻结 DETR-ResNet50 训练主程序。

结构与 ``train_csma_yolo.py`` 一致，将 YOLO 替换为 DETR：
  - L_det：伪 RGB → DETR loss_ce / loss_bbox / loss_giou
  - L_align：DETR encoder 特征 [B, L, 256] + GMM-CMSS
  - Val mAP：``eval_detr_baseline`` 协议

用法：
    bash scripts/01_finetune_detr_final.sh
"""
...
from src.detr_utils import (
    MODEL_ID_DEFAULT,
    collect_cmss_values_detr,
    detr_det_forward_with_feats,
    extract_detr_encoder_features,
    gdino_tensor_to_detr_input,
    labels_to_detr_labels,
    load_frozen_detr,
    run_val_map_detr,
)

_emergency_save_requested = False
_LOG = "[train_csma_detr]"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="CSMA 训练：冻结 DETR-ResNet50 + 可训练 CSMA + GMM-CMSS L_align"
    )
    parser.add_argument(
        "--model-id",
        type=str,
        default=MODEL_ID_DEFAULT,
        help="HuggingFace DETR 模型 id",
    )
    ...
    parser.add_argument("--out-dir", type=str, default="outputs_csma_detr")
    ...
    parser.add_argument("--swanlab-project", type=str, default="csma-detr-training")
    parser.add_argument("--swanlab-run-name", type=str, default="csma-detr-run")
    ...
    print(f"{_LOG} DETR: {args.model_id}")
    ...
    detr_model, detr_proc = load_frozen_detr(args.model_id, device)
    detr_mean = tuple(detr_proc.image_mean)
    detr_std = tuple(detr_proc.image_std)
    print(f"{_LOG} DETR 已加载并冻结")
    ...
    swan_run = _build_swanlab_logger(
        ...
        config={**cfg.to_dict(), "detr_model_id": args.model_id},
    )
    ...
    for epoch in range(start_epoch, cfg.total_epochs):
        ...
        if cmss_sched.should_update_gmm(epoch):
            ...
            cmss_vals = collect_cmss_values_detr(
                detr_model, csma, loader, device,
                img_mean, img_std, detr_mean, detr_std,
        ...
        for batch in loader:
            ...
            with _maybe_autocast(use_amp):
                ...
                detr_pv = gdino_tensor_to_detr_input(
                    pseudo_rgb, img_mean, img_std, detr_mean, detr_std,
                )
                detr_labels = labels_to_detr_labels(labels, device)

                l_det, det_scalars, feat_ir = detr_det_forward_with_feats(
                    detr_model, detr_pv, detr_labels, cfg,
                )
                ...
                if cfg.loss_mode != "det_only" and "rgb_pixel_values" in batch:
                    ...
                    detr_rgb = gdino_tensor_to_detr_input(
                        rgb_pv, img_mean, img_std, detr_mean, detr_std,
                    )
                    with torch.no_grad():
            ...
            if not grad_checked:
                ...
                proj_grad = 0.0
                detr_grads = [p.grad for p in detr_model.parameters()]
                if math.isnan(csma_grad) or math.isinf(csma_grad):
                    print(f"[梯度检查] 跳过（csma_grad={csma_grad:.6g}）")
                else:
                    assert csma_grad > 0.0, "CSMA 梯度为 0"
                    assert all(g is None for g in detr_grads), "冻结 DETR 不应有梯度"
                    print(f"[梯度检查] CSMA grad L1={csma_grad:.6f}  DETR grad=None: OK")
                    grad_checked = True
        ...
        _save_csma_weights(epoch_ckpt, csma)
        ...
        if (
            ...
            metrics = run_val_map_detr(
                csma, detr_model, detr_proc, processor,
                val_dataset, val_valid_ids, device,
                args.val_batch_size, args.val_conf,
            )
            metrics["epoch"] = epoch
            metrics["stage"] = stage
