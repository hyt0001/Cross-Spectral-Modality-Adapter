...
class PixelDecoder(nn.Module):
    ...
    def forward(
        ...
        x = self.up3(feat)
        # stride-2 编码器做 floor 除法，ConvTranspose2d 做严格 2× 乘法，
        # 奇数尺寸时二者差 1 像素（如 171 vs 172）——用 skip 的真实尺寸强制对齐
        if x.shape[2:] != skip_c2.shape[2:]:
            x = F.interpolate(x, size=skip_c2.shape[2:], mode="nearest")
        x = torch.cat([x, skip_c2], dim=1)
        x = self.fuse3(x)
        x = self.up2(x)
        if x.shape[2:] != skip_c1.shape[2:]:
            x = F.interpolate(x, size=skip_c1.shape[2:], mode="nearest")
...
class CSMA(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        delta = self.pd(feat_map, c2, c1)           # tanh 输出，范围 [-1, 1]
        # ConvTranspose2d 在奇数输入尺寸时输出可能比输入多 1 pixel（如 947→948），
        # 与上游一致：插值回输入的空间尺寸再做残差相加
        if delta.shape[-2:] != x.shape[-2:]:
            delta = torch.nn.functional.interpolate(
                delta, size=x.shape[-2:], mode="bilinear", align_corners=False
            )
        if self.cfg.use_residual:
            # residual_scale 控制 CSMA 对输入的改动幅度（小 → 更保守）
            pseudo_rgb = delta * self.cfg.residual_scale + x
        else:
            pseudo_rgb = delta
        # pseudo_clamp > 0 时限制像素极端值，抑制亮块假框（0 = 不 clamp）
        if self.cfg.pseudo_clamp > 0:
            pseudo_rgb = pseudo_rgb.clamp(-self.cfg.pseudo_clamp, self.cfg.pseudo_clamp)
        return pseudo_rgb
