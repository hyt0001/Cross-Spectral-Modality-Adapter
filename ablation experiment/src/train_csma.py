...
from src.csma_variants import CSMAMeanProto
...
from src.dataset_llvip import LLVIPPairedDataset, build_llvip_category_map, collate_llvip
from src.dataset_m3fd import M3FDPairedDataset, build_m3fd_category_map, collate_m3fd
...
def _run_val_map(
    ...
    coco_gt = _build_gt_coco(val_dataset, valid_cat_ids, dataset_mode=dataset_mode)
    csma.eval()
    try:
...
def _compute_tv_loss(x: torch.Tensor) -> torch.Tensor:
    """
    各向同性 Total Variation 正则：鼓励 pseudo_rgb 空间平滑、减少噪点假框。

    参数:
        x: [B, C, H, W] pseudo RGB tensor。
    返回:
        标量 TV loss（归一化到像素数）。
    """
    diff_h = x[:, :, 1:, :] - x[:, :, :-1, :]
...
def _build_det_loss(
    ...
    _debug_det = os.environ.get("DEBUG_DET_LOSS", "0") == "1"
    if _debug_det:
        for _k, _v in ld.items():
            if torch.is_tensor(_v):
                try:
                    print(f"[det_loss_debug] {_k} = {float(_v.detach().cpu())}")
                except Exception:
                    print(f"[det_loss_debug] {_k} = tensor shape={tuple(_v.shape)}")
    loss_det = (
        ld["loss_ce"]
...
def main() -> None:
    ...
    parser.add_argument("--dataset",       type=str, default="flir_v1",
                        choices=["legacy", "flir_v1", "flir_v2", "m3fd", "llvip"],
                        help="数据集类型：flir_v1=FLIR_License 配对数据集（默认）；"
                             "flir_v2=FLIR_ADAS_v2 无配对；legacy=旧版 train/ 目录；"
                             "m3fd=M3FD 多光谱配对；"
                             "llvip=LLVIP 可见光-红外配对（仅 person 类，需先运行 convert_llvip_to_coco.py）")
    ...
    parser.add_argument("--num-workers",   type=int, default=None,
                        help="DataLoader num_workers（0=单进程，smoke test / 容器内建议用 0）")
    ...
    parser.add_argument("--llvip-split",   type=str, default="train",
                        choices=["train", "test"],
                        help="LLVIP split（仅 --dataset llvip 时生效，默认 train）")
    ...
    # ── Pseudo-RGB 正则参数（Final Model）──────────────────────────────────────
    parser.add_argument("--id-loss-weight",   type=float, default=None,
                        help="L_id 权重：MSE(pseudo_rgb, ir)（默认 0.05）")
    parser.add_argument("--tv-loss-weight",   type=float, default=None,
                        help="L_tv 权重：Total Variation 平滑正则（默认 0.01）")
    parser.add_argument("--logit-reg-weight", type=float, default=None,
                        help="L_logit 权重：mean(pred_logits²) 抑制高分框（默认 0.01）")
    parser.add_argument("--pseudo-clamp",     type=float, default=None,
                        help="pseudo_rgb 像素值截断上限（0=不 clamp；默认 3.0）")
    parser.add_argument("--residual-scale",   type=float, default=None,
                        help="CSMA 残差缩放系数（默认 0.1；Final Model=0.05）")
    # ── 消融实验参数（Ablation B/C）────────────────────────────────────────────
    parser.add_argument(
        "--cmss-ablation-mode", type=str, default="none",
        choices=["none", "random_mask", "fixed_threshold", "gmm_single_b"],
        help=(
            "消融实验模式（默认 none=主实验完整路径）：\n"
            "  none            — 三阶段 GMM-CMSS A→B→C（主实验 C-4）\n"
    ...
    parser.add_argument(
        ...
        choices=["mean_proto"],
        help="模型变体（默认 None=标准 CSMA）：mean_proto=Ablation B-2 均值原型变体",
    )
    parser.add_argument(
        "--mean-proto-path", type=str, default=None,
        help="均值原型文件路径（--variant mean_proto 时必须提供，由 compute_mean_proto.py 生成）",
    )
    ...
    if args.pseudo_clamp      is not None: overrides["pseudo_clamp"]       = args.pseudo_clamp
    ...
    # m3fd：有 RGB（vi/）配对，默认 full（同 flir_v1）
    # legacy：使用用户指定值
    if args.dataset in ("flir_v1", "m3fd", "llvip"):
    ...
    # 消融实验模式写入 config（默认 "none" 不影响原有行为）
    overrides["cmss_ablation_mode"] = args.cmss_ablation_mode
    ...
    # Ablation B-2：用 CSMAMeanProto 替代标准 CSMA（仅当 --variant mean_proto 时）
    if getattr(args, "variant", None) == "mean_proto":
        if not args.mean_proto_path:
            raise ValueError("--variant mean_proto 时必须提供 --mean-proto-path（由 compute_mean_proto.py 生成）")
        mean_proto_tensor = torch.load(args.mean_proto_path, map_location=device)
        csma: nn.Module = CSMAMeanProto(cfg, mean_proto_tensor).to(device)
        print(f"[train_csma] 使用 Ablation B-2 变体（CSMAMeanProto），均值原型来自: {args.mean_proto_path}")
    else:
    ...
    if args.dataset == "flir_v1":
        ...
        cat_map, valid_ids = build_m3fd_category_map(cfg.text_prompt)
        dataset = M3FDPairedDataset(
            ...
            category_map=cat_map,
            valid_cat_ids=valid_ids,
            # 训练时统一到 1024×768（M3FD 主流分辨率），保证 batch 同形；
            # 评测时保持原始分辨率（collate 补边对齐）
            canonical_size=(1024, 768),
        )
        collate_fn_use = collate_m3fd
        print(f"[train_csma] 数据集模式: m3fd  loss_mode={cfg.loss_mode}")
    elif args.dataset == "llvip":
        cat_map, valid_ids = build_llvip_category_map(cfg.text_prompt)
        _llvip_split = getattr(args, "llvip_split", "train")
        ...
        collate_fn_use = collate_llvip
        print(f"[train_csma] 数据集模式: llvip  split={_llvip_split}  loss_mode={cfg.loss_mode}")
    else:
    ...
    for epoch in range(start_epoch, cfg.total_epochs):
        ...
        # Phase 5.7：GMM 定期更新
        # Ablation C-1（random_mask）和 C-2（fixed_threshold）不需要 GMM，直接跳过
        _skip_gmm = cfg.cmss_ablation_mode in ("random_mask", "fixed_threshold")
        _need_resume_gmm = (
            (not _skip_gmm)
            and cmss_sched.gmm is None
            and epoch == start_epoch
        )
        if not _skip_gmm and (cmss_sched.should_update_gmm(epoch) or _need_resume_gmm):
            if _need_resume_gmm:
                print(f"[train_csma] Epoch {epoch}: 续训检测到 GMM 为空，先重新拟合 GMM...")
            else:
        ...
        # Ablation C-1/C-2/C-3 固定 λ=0.5/0.5；主实验（"none"）使用动态权重
        if cfg.cmss_ablation_mode in ("random_mask", "fixed_threshold", "gmm_single_b"):
            lambda_align, lambda_det = 0.5, 0.5
        else:
        ...
        for batch in loader:
            ...
            with _maybe_autocast(use_amp):
                ...
                # 消融分支通过 cfg.cmss_ablation_mode 控制掩码策略（默认 "none" 走主实验路径）
                ...
                if "rgb_pixel_values" in batch and "feat" in _feat_ir_cache:
                    ...
                    _abl_mode = cfg.cmss_ablation_mode
                    if _abl_mode == "random_mask":
                        # Ablation C-1：跳过 CMSS，直接随机生成掩码
                        # L 维度动态获取（DINO 多尺度拼接，非硬编码）
                        B_cur, L_cur = feat_ir.shape[0], feat_ir.shape[1]
                        len_keep = int(L_cur * (1.0 - cfg.mask_ratio))
                        mask = torch.ones(B_cur, L_cur, device=device, dtype=torch.float32)
                        rand_ids = torch.argsort(
                            torch.rand(B_cur, L_cur, device=device), dim=-1
                        )
                        mask.scatter_(1, rand_ids[:, :len_keep], 0.0)
                    elif _abl_mode == "fixed_threshold":
                        # Ablation C-2：CMSS 固定阈值 μ₂=0.5，不依赖 GMM
                        cmss_map = compute_cmss(feat_rgb.detach(), feat_ir.detach())
                        mask = build_cmss_mask(
                            cmss_map, stage=0,                 # 硬编码 Stage A 逻辑
                            mu1=0.2, mu2=0.5, mu3=0.8,         # 固定先验，不用 GMM 均值
                            mask_ratio=cfg.mask_ratio,
                            gmm=None,                          # 不使用 GMM
                        )
                    elif _abl_mode == "gmm_single_b":
                        # Ablation C-3：GMM 动态阈值 + 固定 Stage B（不切换阶段）
                        # 若 GMM 尚未拟合（epoch 0 拟合失败或数据不足），退回 Stage A 固定阈值，
                        # 避免 build_cmss_mask(stage=1, gmm=None) 抛出 ValueError。
                        cmss_map = compute_cmss(feat_rgb.detach(), feat_ir.detach())
                        if cmss_sched.gmm is None:
                            print(
                                "[WARN] gmm_single_b fallback to fixed threshold "
                                "because gmm is None"
                            )
                            mask = build_cmss_mask(
                                cmss_map, stage=0,
                                mu1=0.2, mu2=0.5, mu3=0.8,
                                mask_ratio=cfg.mask_ratio,
                                gmm=None,
                            )
                        else:
                            mask = build_cmss_mask(
                                cmss_map, stage=1,             # 硬编码 Stage B
                                mu1=mu1, mu2=mu2, mu3=mu3,
                                mask_ratio=cfg.mask_ratio,
                                gmm=cmss_sched.gmm,
                            )
                    else:
                        # 主实验（"none"）：三阶段动态路径，原逻辑不变
                        cmss_map = compute_cmss(feat_rgb.detach(), feat_ir.detach())
                        mask = build_cmss_mask(
                            cmss_map, stage,
                            mu1, mu2, mu3,
                ...
                # Phase 5.10b：Pseudo-RGB 正则 loss（Final Model）
                # L_id: pseudo 像素贴近原始 IR（weight 小 → 放松重建约束）
                # L_tv: Total Variation 平滑 pseudo，抑制噪点假框
                # L_logit: MSE 正则化 pred_logits，压制满屏高分框
                _z = torch.tensor(0.0, device=device)
                l_id = (
                    torch.nn.functional.mse_loss(pseudo_rgb, ir_pv)
                    if cfg.id_loss_weight > 0 else _z
                )
                l_tv = (
                    _compute_tv_loss(pseudo_rgb)
                    if cfg.tv_loss_weight > 0 else _z
                )
                l_lr = (
                    # 上游定义（pseudo_losses.logit_regularization）：
                    # 对检测 logits 的平均 sigmoid 施加惩罚，压低整体置信度、
                    # 减少满屏假框；值域 (0,1)，梯度有效（平方均值会溢出为常数）
                    outputs.logits.float().sigmoid().mean()
                    if cfg.logit_reg_weight > 0 else _z
                )
            ...
            if ep_step % 10 == 0 or ep_step == 1:
                print(
                    ...
                    f"align={align_val:.5f}  "
                    f"id={id_val:.5f}  tv={tv_val:.5f}  logit_reg={lr_val:.5f}  "
            ...
            if swan_run is not None:
                swan_run.log({
                    ...
                    "train/loss_logit_reg_step": lr_val,
        ...
        mean_id    = float(np.mean(ep_loss_id))
        mean_tv    = float(np.mean(ep_loss_tv))
        mean_lr    = float(np.mean(ep_loss_lr))
        ...
        print(
            ...
            f"id={mean_id:.5f}  tv={mean_tv:.5f}  logit_reg={mean_lr:.5f})  "
        ...
        if swan_run is not None:
            swan_run.log({
                ...
                "train/loss_id_epoch":        mean_id,
                "train/loss_tv_epoch":        mean_tv,
                "train/loss_logit_reg_epoch": mean_lr,
